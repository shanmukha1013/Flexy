...
class Profile {
    ...
    renderProfileInfo() {
        ...
        document.getElementById('profile-phone').textContent = `+91 ${this.currentUser.phone}`;
    ...
    showEditProfileModal() {
        ...
        const modal = document.getElementById('edit-profile-modal'); if (modal) { modal.style.display = 'flex'; modal.setAttribute('aria-hidden','false'); }
        // populate live preview
        this.updateProfilePreview();
    }

    hideEditProfileModal() {
        const modal = document.getElementById('edit-profile-modal'); if (modal) { modal.style.display = 'none'; modal.setAttribute('aria-hidden','true'); }
        // clear fields (keep avatar until saved)
        ['edit-name','edit-phone','edit-bio','edit-location'].forEach(id=>{ const el=document.getElementById(id); if(el) el.value=''; });
    }
