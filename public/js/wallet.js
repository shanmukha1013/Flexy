...
class Wallet {
    ...
    renderCardDetails() {
        ...
        if (holderEl && this.currentUser) {
            holderEl.textContent = this.currentUser.name || 'Member';
        }
    ...
    renderWalletChart() {
        ...
        let runningBalance = this.currentUser ? this.currentUser.balance : 10000;
        
        // Compute historical balances
        const valuesRev = [];
        valuesRev.push(runningBalance);
        ...
        for (let i = 0; i < points - 1; i++) {
            ...
            if (tx) {
                ...
                runningBalance -= 500; // seed fallback delta
            }
            valuesRev.push(Math.max(0, runningBalance));
        }

        const balancePoints = valuesRev.reverse();
        ...
        const maxVal = Math.max(...balancePoints, 1000) * 1.15;
        const minVal = 0;
        ...
        if (balancePoints.length > 0) {
            ...
            for (let i = 0; i < points; i++) {
                const x = paddingLeft + (chartWidth / (points - 1)) * i;
                const y = paddingTop + chartHeight - ((balancePoints[i] - minVal) / (maxVal - minVal)) * chartHeight;
            ...
            // Pulsing dot on final balance
            const x = paddingLeft + (chartWidth / (points - 1)) * (points - 1);
            const y = paddingTop + chartHeight - ((balancePoints[points - 1] - minVal) / (maxVal - minVal)) * chartHeight;
