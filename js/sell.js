...
class Sell {
    ...
    validateStep(step) {
        switch(step) {
            ...
            case 3: // Pricing
                if (this.itemData.priceType !== 'showcase') {
                    if (!this.itemData.price || this.itemData.price < 1) {
                        this.showNotification('Please enter a valid starting bid', 'error');
                        return false;
                    }
                }
    ...
    toggleAuctionSettings() {
        ...
        // Dynamically adjust labels for social showcases vs active auctions
        const priceLabel = document.querySelector('label[for="item-price"]');
        const publishText = document.getElementById('publish-text');
        
        if (priceLabel) {
            if (this.itemData.priceType === 'showcase') {
                priceLabel.textContent = 'Estimated Valuation (₹) (Optional)';
            } else if (this.itemData.priceType === 'auction') {
        ...
        if (publishText) {
            if (this.itemData.priceType === 'showcase') {
                publishText.textContent = 'Publish Showcase Post';
            } else {
                publishText.textContent = 'Publish Auction Item';
            }
        }
    }
    ...
    saveItem() {
        ...
        const itemPayload = {
            ...
            likedBy: [],
            comments: [],
            story: this.itemData.priceType === 'showcase' ? this.itemData.description : ''
        };
